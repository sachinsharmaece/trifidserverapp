...
export type ErrorCode =
  ...
  | 'RATE_LIMITED'
  | 'VALIDATION_FAILED'
  // M4 additions — BUSINESS_RULES.md §2–§4, §17. A technical gap in the
  // fixed API_CONTRACT.md §10 list, same reasoning as INVALID_CREDENTIALS
  // above: these name a specific chain/money refusal, not a business rule.
  | 'MARGIN_CELL_MISSING'
  | 'CHAIN_STAGE_GUARD_FAILED'
  | 'PO_ALREADY_EXISTS'
  | 'DOCUMENT_ALREADY_BILLED'
  | 'INSPECTION_IMMUTABLE'
  | 'REFUND_DESTINATION_MISMATCH'
  | 'DAY_CLOSE_OUT_OF_BALANCE'
  // M5 additions — modules/orders. A technical gap in the fixed
  // API_CONTRACT.md §10 list, same reasoning as MARGIN_CELL_MISSING above.
...
const HTTP_STATUS_BY_CODE: Record<ErrorCode, number> = {
  ...
  MARGIN_CELL_MISSING: 422,
  CHAIN_STAGE_GUARD_FAILED: 409,
  PO_ALREADY_EXISTS: 409,
  DOCUMENT_ALREADY_BILLED: 409,
  INSPECTION_IMMUTABLE: 409,
  REFUND_DESTINATION_MISMATCH: 409,
  DAY_CLOSE_OUT_OF_BALANCE: 409,
